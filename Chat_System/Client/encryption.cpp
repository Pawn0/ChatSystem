...
std::string Encryption::encrypt ( const std::string& item ) {
    ...
    CryptoPP::StringSource stringSource ( item, true, new CryptoPP::PK_EncryptorFilter ( rng, e, new CryptoPP::StringSink ( cipher ) ) );
...
bool Encryption::keysPresent() {
    ...
    if ( boost::filesystem::exists ( "../keyFolder" )&&boost::filesystem::is_regular_file("../keyFolder/privateKey" )&& boost::filesystem::is_regular_file("../keyFolder/publicKey")) {
...
void Encryption::loadFromFolder (const std::string& fileName, CryptoPP::RSA::PublicKey& publicKey){
...
void Encryption::loadFromFolder(const std::string& fileName, CryptoPP::RSA::PrivateKey& privateKey){
    try{//load the data from the folder
        CryptoPP::ByteQueue queue;
        load(fileName, queue);
...
void Encryption::load(const std::string& fileName, CryptoPP::BufferedTransformation& buffTrans){
...
void Encryption::save ( const std::string& fileName, const CryptoPP::BufferedTransformation& bt ) {
