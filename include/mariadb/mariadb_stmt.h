...
{
  my_bool (*prepare) (const MYSQL_STMT * stmt, const char *const query,
		      size_t query_len);
  ...
  MYSQL_RES *(*get_result) (const MYSQL_STMT * stmt);
    ...
    my_bool (*fetch) (const MYSQL_STMT * stmt,
		      my_bool * const fetched_anything);
    ...
    my_bool (*send_long_data) (const MYSQL_STMT * stmt,
			       unsigned int param_num, const char *const data,
			       size_t length);
  ...
  const char *(*get_sqlstate) (const MYSQL_STMT * stmt);
    ...
    my_bool (*get_attribute) (const MYSQL_STMT * stmt,
			      enum enum_stmt_attr_type attr_type,
			      const void *value);
    my_bool (*set_attribute) (const MYSQL_STMT * stmt,
			      enum enum_stmt_attr_type attr_type,
			      const void *value);
  void (*set_error) (MYSQL_STMT * stmt, unsigned int error_nr,
		     const char *sqlstate, const char *format, ...);
};

typedef int (*mysql_stmt_fetch_row_func) (MYSQL_STMT * stmt,
					  unsigned char **row);
...
typedef void (*ps_field_fetch_func) (MYSQL_BIND * r_param,
...
int STDCALL mariadb_stmt_execute_direct (MYSQL_STMT * stmt,
					 const char *stmt_str, size_t length);
