...
#define MYSQL_CLIENT_PLUGIN_HEADER                      \
  ...
  struct st_mysql_client_plugin *mysql_load_plugin (struct st_mysql *mysql,
						    const char *name,
						    int type, int argc, ...);
  struct st_mysql_client_plugin *STDCALL mysql_load_plugin_v (struct st_mysql
							      *mysql,
							      const char
							      *name, int type,
							      int argc,
							      va_list args);
  struct st_mysql_client_plugin *STDCALL mysql_client_find_plugin (struct
								   st_mysql
								   *mysql,
								   const char
								   *name,
								   int type);
  struct st_mysql_client_plugin *STDCALL mysql_client_register_plugin (struct
								       st_mysql
								       *mysql,
								       struct
...
/* Should definitely be used if one uses shared libraries */
  ...
  my_bool mariadb_get_infov (MYSQL * mysql, enum mariadb_value value,
			     void *arg, ...);
  my_bool STDCALL mariadb_get_info (MYSQL * mysql, enum mariadb_value value,
				    void *arg);
  ...
  MARIADB_CHARSET_INFO *STDCALL mariadb_get_charset_by_name (const char
							     *csname);
...
/* Async API */
  ...
  int STDCALL mysql_autocommit_cont (my_bool * ret, MYSQL * mysql,
				     int status);
  ...
  int STDCALL mysql_reset_connection_cont (int *ret, MYSQL * mysql,
					   int status);
  ...
  int STDCALL mysql_stmt_prepare_start (int *ret, MYSQL_STMT * stmt,
					const char *query,
					unsigned long length);
  int STDCALL mysql_stmt_prepare_cont (int *ret, MYSQL_STMT * stmt,
				       int status);
  int STDCALL mysql_stmt_execute_start (int *ret, MYSQL_STMT * stmt);
  int STDCALL mysql_stmt_execute_cont (int *ret, MYSQL_STMT * stmt,
				       int status);
  ...
  int STDCALL mysql_stmt_free_result_cont (my_bool * ret, MYSQL_STMT * stmt,
					   int status);
  int STDCALL mysql_stmt_send_long_data_start (my_bool * ret,
					       MYSQL_STMT * stmt,
					       unsigned int param_number,
					       const char *data,
					       unsigned long len);
  int STDCALL mysql_stmt_send_long_data_cont (my_bool * ret,
					      MYSQL_STMT * stmt, int status);
...
/* API function calls (used by dynmic plugins) */
  struct st_mariadb_api
  {
    ...
    int (STDCALL * mysql_set_character_set) (MYSQL * mysql,
      ...
      my_bool (*mariadb_get_infov) (MYSQL * mysql, enum mariadb_value value,
				    void *arg, ...);
      my_bool (STDCALL * mariadb_get_info) (MYSQL * mysql,
    ...
    unsigned long (STDCALL * mysql_get_client_version) (void);
      my_bool (STDCALL * mariadb_connection) (MYSQL * mysql);
    const char *(STDCALL * mysql_get_server_name) (MYSQL * mysql);
    MARIADB_CHARSET_INFO *(STDCALL *
			   mariadb_get_charset_by_name) (const char *csname);
    MARIADB_CHARSET_INFO *(STDCALL *
      ...
      size_t (STDCALL * mariadb_convert_string) (const char *from,
						 ...
						 int *errorcode);
    int (*mysql_optionsv) (MYSQL * mysql, enum mysql_option option, ...);
    int (*mysql_get_optionv) (MYSQL * mysql, enum mysql_option option,
			      void *arg, ...);
    int (STDCALL * mysql_get_option) (MYSQL * mysql, enum mysql_option option,
				      void *arg);
    unsigned long (STDCALL * mysql_hex_string) (char *to, const char *from,
...
/* these methods can be overwritten by db plugins */
  struct st_mariadb_methods
  {
    ...
    int (*db_command) (MYSQL * mysql, enum enum_server_command command,
		       const char *arg, size_t length, my_bool skipp_check,
		       void *opt_arg);
    ...
    void (*set_error) (MYSQL * mysql, unsigned int error_nr,
		       const char *sqlstate, const char *format, ...);
