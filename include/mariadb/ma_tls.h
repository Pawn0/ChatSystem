...
/* ma_tls_write
   ...
   Returns:
     ...
     -1           if an error occurred
*/
ssize_t ma_tls_write (MARIADB_TLS * ctls, const uchar * buffer,
		      size_t length);
...
*/
unsigned int ma_tls_get_finger_print (MARIADB_TLS * ctls, char *fp,
				      unsigned int fp_len);
...
ssize_t ma_pvio_tls_read (MARIADB_TLS * ctls, const uchar * buffer,
			  size_t length);
ssize_t ma_pvio_tls_write (MARIADB_TLS * ctls, const uchar * buffer,
			   size_t length);
...
my_bool ma_pvio_tls_check_fp (MARIADB_TLS * ctls, const char *fp,
			      const char *fp_list);
